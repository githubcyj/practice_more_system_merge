...
export function handleCustomColumns(data, _this) {
  return data.map(item => {
    ...
    return {
      ...
      onCell: record => {
        let initialValue = record[item.customizeField] && record[item.customizeField].trim() || undefined
        let props = {}
        const rules = []
        ...
        if(item.fieldType == 'select-multiple') {
          ...
          if(record[item.customizeField] && record[item.customizeField].trim()) {
        ...
        if(item.fieldType == 'date') {
          initialValue = record[item.customizeField] && record[item.customizeField].trim() && moment(Number(record[item.customizeField])) || undefined
        }
